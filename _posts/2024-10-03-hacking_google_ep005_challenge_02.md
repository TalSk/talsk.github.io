...
Besides the game on the main endpoint, on failing and scoring high enough, it presents an option to be added to the High Scores page, and then displays that page until refresh.
...
- `POST` to `/api/highscores` with the paramters `name`, `score` and `signature`. The response congratulates you on your place on the leaderboards.
- `GET` to `/api/highscores`, returning the high score list.

After quick testing, it seems that you are only able to be successfully added to the high scores is by having a vaid signature. The server happily signs even very high scores, and there's no much indication on what I should achieve here. 

I then noticed that the challenge gives a hint:

> Hint: Can you score lower than zero?

Testing it out, the server doesn't agree to sign a negative score. Maybe it's some underflow issue? 
...
The handler takes a `name`, `score` and `signature` parameters, and it accepts a `score` smaller than 0 since the verification check is: `if type(score) != int or not -2**16 <= score < 2**16`.
...
Within `verify`, using the class `VerifyingKey` that verifies RSA signatures, the server checks that the signature of `json.dumps([KEY_ID, name, score])` matches the given signature, where `KEY_ID` is a public key it takes from a local file.
...
Inspecting its handler code, it just a `name` and `score` parameters and signs them using the RSA private key corresponding to the public key is used to verify the password. However in *this handler*, the verification check on `score` is: `if type(score) != int or score < 0`.

So, I can freely sign a score which is any positive integer, but I somehow need to get a valid signature for a score which is a negative integer.

I tried and tested some ways to sneak in a negative integer to be signed.

It can't be something with a large positive integer, as Python has no problem storing arbitrarily large integer. Also, I couldn't really affect what string is signed since the message being signed is `json.dumps([KEY_ID, name, score])`. For an example for `name="nam"` and `score=5` the message will be (the string) `'["pzero-adventures", "nam", 5]'`.
...
Anyhow, with that realization, the main focus should be `crypto.py` - a file the server uses to sign and verify, instead of relying on a library (another sign alluding to a crypto challenge I missed earlier).
...
A comment on top of the `sign` and `verify` functions directed me to RFC2313, which describes a cryptographic system using RSA encryption. Specifically, it defines version 1.5 of [PKCS#1](https://en.wikipedia.org/wiki/PKCS_1) (Stands for "First Public-Key Cryptography Standard").

This family of standards includes several primitives - encryption, description, signing and verification.

A classic move with cryptography challenges you have no clue about (which was my situation) - you search for known attacks on the crypto system that is used here.

The search query: "PKCS #1 v1.5 attack" leads to multiple articles about "Bleichenbacher attack". That's a good sign, and it's time to do some reading.
...
This information is enough that, using about a few million queries (not much for a modern computer), the RSA private key could be reconstructed. And that's how it is with crypto systems - a seemnigly tiny amount of information might be the flaw to bring the entire system down.
...
I found that in 2006, [a variant of this attack was presented by Bleichenbacher](https://ieeexplore.ieee.org/document/4159923), showed how, in cases where the RSA public key (`e`) is `3`, it was possible to easily forge fake signature.

Hey, I need to forge PKCS #1 v1.5 signatures! To verify the conncection, I quickly queried the server at `api/keys` which revealed that the server's public key is indeed `3`:
...
As a quick tl;dr for the upcoming section: Bleichenbacher exploited the fact that many verifies, taking the plaintext signature after decryption just check that it starts with the correct 2-byte header, skip over the padding and parse the following bytes as the siganture. 

By skipping over the padding and not checking that it indeeds pads properly, verifiers essentially only looked at a small prefix of the plaintext signature. When the public key is small, we can quite easily forge signatures beginning with whatever we desire. 

Let's see this! Starting off, we know that the RSA public key (`e`) is `3`. 

The signature verification function starts by taking encrypted signature and decrypt it. In an RSA system, it means raising the signature (in integer form) to the power of the public key and calculating the result modulo the modulus `n` (= dividing by `n` and taking the remainder).

So for our public key, the verification function recieves an encrypted signature and just raises it to the 3rd power (AKA cubing it).

```python
k = pow(s, self.e, self.n) # s is the signature, e is the public key, n is the modulos
```

In case the encypted signature `s` is a small integer (less than the cube root of `n`), then the result of this operation will be smaller than `n`, meaning no modulus operation happens. In case the verifier only cares about the *beginning* of the plaintext signature, this behavior is risky.

Why? Theoretically, if we want this decryption operation to output a certain signature, we can start from this number and simply take its cube root, then pass it to the verificaton function as the encrypted signature.

This obviously doesn't always work - we're only working with whole integers, so the chances our signature's cube root is an exact integer are very very very small.

However, if we just round the root cube result to the next integer, then the cube of that integer won't be too far from the number we started from - so a prefix of the it won't be affected!

This can be easily seen by an example: let's imagine we're working with 8-digit decimal integers, and we want want our signature to start with `17`. 
...
So we have the "ability" to forge signatures with Bleichenbacher's attack, how can it help me break the challenge? 
...
As I mentioend above, after decrypting the signature, verifiers didn't bother checking that the padding really is large enough to pad the entire signature to be 256 characters long.

Instead, they seperated the signature on the null bytes (`0x00`), verified the minimum amount of padding (`8`) and then moved on to parse the signature. If we count the number of bytes of such a "short" version of a signature, we have:

```
2 (Header bytes) + 8 (Padding) + 1 (Separator) + 0x33 (DerSequence - 0x31 + 2 header bytes)
```

For a total of 62 bytes, leaving us 256-62=194 bytes to play with, so the "prefix attack" is very feasible!

Glacing quickly at the `verify` function the server users, I saw this:
...
def forge_signature(message):
    ...
    # Verifying the signature leads to the required plaintext
...
And it works - the assert passes without a problem!

Out of interest, I looked how each part looks like. Here's the required prefix (as hex):

```
0001ffffffffffffffff003031300d060960864801650304020105000420707eca926424f185c834f219d311950df8bdfa8a83fc2e3b36e6c2210450853e
```

Here's the encrypted signature (as an integer)

```
995391042663285905082373840334783075535901580439276937985819219650252243000986193655522714919094035649842008832283737311056527766480523975681564515126086150333565353700999252276489952678424776872790802557
```

And here's the decrypted signature (after taking the cube of the previous integer)
...
0x00 # Separator
0x30 0x31 # Outer DerSequence
0x30 0x0d # Inner DerSequence
0x06 0x09 0x608648016503040201 # AlgorithmObjectId
0x05 0x00 # Null
0x04 0x20 0x707eca926424f185c834f219d311950df8bdfa8a83fc2e3b36e6c2210450853e # OctetStringHash
...
As can be easily seen, it starts with the prefix I wanted, and there are even multiple extra 0s to spare after the hash!

I spun a local version of the server and sent the signature to the server, and it worked! Kinda'.

This signature passes the padding check, but when the code executes `sequence = DerSequence(); sequence.decode(digest_info)`, it crashes on with a `ValueError: Unexpected extra data after the DER structure` error. 

The `digest_info` variables holds a string that starts with a correct DER stucture, but ends with garbage. Unfortuantely, the Python implemention of `DerSequence` doesn't really care the reported length is `0x31`, and won't peacefully accept the garbage.
...
Ah, I could increase the prefix just a little bit, and make the outer `DerSequence` to include yet another object and look something like that:
...
Well, this idea *could* work if I instead put the garbage in the first *inner* `DerSequence`, so the outer `DerSequence` look like this:
...
However, this breaks Bleichenbacher's "prefix trick" - since the signature needs the hash as its suffix, which would be destroyed by the attack.

I did wonder if someone had already bumped into this issue. online, I bumped into [this neat article from 2016 by Filippo Valsorda](https://words.filippo.io/bleichenbacher-06-signature-forgery-in-python-rsa/), who discovered the `python-rsa` module is vulnerable to signature forgery.

The verification code was not vulnerable to Bleichenbacher's "prefix trick", but rather the issue was that it didn't validate that the padding bytes are actually composed of 0xFF, so Filippo's idea was to hide the garbage in the padding, leading him to the same path of having including the `OctetStringHash` object in the end!
...
So, while not exactly the case I have here, if I learn how Filippo managed to have the required `OctetStringHash` suffix in the final signature, I can still hide the garbage in the inner `DerSequence` instead.

After reading the article, honestly the trick is nothing short of magic. And although it's explained really well, I'll give it a shot here too.
...
The theory is that if we *simply replace* the last bytes of this encrypted siganture with the encrypted signature suffix we just found, nothing will happen to the prefix after decryption!
...
Time for showing how this works with a practical example: let's say we want our signature to end with `0x40000000`. Taking a cube root, our encrypted siganture suffix should be `0x400`.

Before, when we constructed the encrypted signature with a chosen prefix, we ended up with the following:

```py
prefix_signature = 995391042663285905082373840334783075535901580439276937985819219650252243000986193655522714919094035649842008832283737311056527766480523975681564515126086150333565353700999252276489952678424776872790802557
```

Let's convert it to bytes and replace its suffix with `0x400`:

```py
new_signature = int.from_bytes(bytes.fromhex(prefix_signature.to_bytes(256, 'big').hex()[:-3] + '400'), 'big')
```

Now, if we decrypt the new signature:

```py 
print(binascii.hexlify(pow(new_signature, 3).to_bytes(2048//8, 'big')))
```
...
(In some cases, depending on the exact suffix, the conversion might "carry" a bit over and so the last bit of the prefix will change as well as the null bytes become a bunch of `ff`s

To avoid this, we can just pad with random values:
...
So now we have this "suffix magic trick". But, did you notice I cheated? I chose a suffix with a perfect integer cube root. So given an arbitrary suffix (whose cube root is not an integer), how can we find what its corresponding encrypted suffix?

Since it doesn't have a perfect cube root, we'll have to a *longer* suffix with an integer cube root that *ends with* the required suffix. That sounds difficult? Apprenatly not! It only takes a simple iterative process that slowly discovers the required encrypted signature suffix and works as follows:

We start with the encrypted signature suffix as the `1`. Treating it as a long binary string with `...00001`, we cube it and see which bits of the output match the required siganture suffix, going from least significant to most significant bits.

When we bump into a mismatch at some index, we flip the bit at this index in the encrypted signature suffix.

This has a twofold effect: the cube's bit also flips, meaning now this index matches. Additionally, since cubing is basically a bunch of multplications, changing the bit at some bit index doesn't effect the output bits that are less significant to this bit (recall long multiplication from school - the later rows in the result were pushed to the left, making it so they don't affect previous indices).
...
But since our required suffix is so small relative to `n`, we're basically promised it succeeds. Filippo has a nice diagram showing this with a practical example:

Let's imainge our required suffix is `0b1010101101`. Here's the iterative process:
...
The final encrypted signature suffix is `0b1010010101`. It's decrypted version ends with `1010101101` prepended with some bits we don't care about.
...
On the suffix side, since it is inserted as a suffix to a larger integer, the more significant bits won't affect the decrypted version this suffix, same as what we exploited in the iterative process.

One note that the plaintext signature has to start with `1` (due to the iterative process starting with `1`). This means we can only aim for odd suffixes. However since the hash is a SHA256 of a value we control (the name), we can easily manually find an  odd one.
...
def forge_signature_2(message):
    ...
    plaintext_suffix = b'\x04\x20' + digest # OctetString - type 0x04, legnth 0x20
    ...
    # Construct the encrypted signature with prefix
...
```
0001ffffffffffffffff003081f23081cd060960864801650304020105000481bd // Prefix
04205c354e41c261f1f569f1762a999ab8ae7250d742c41075c7f33b4d776f574d55 // Suffix
```

And here's the resulting signature's decryption:

```

```
